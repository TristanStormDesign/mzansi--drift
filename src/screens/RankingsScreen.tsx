  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        ...
        const fetchedPlayers: Player[] = snapshot.docs.map((docSnap) => {
          const data = docSnap.data();
          return {
            id: docSnap.id,
            username: (data.username as string) || 'Unknown',
            countryFlag: (data.profilePhoto as string) || null,
  ...
  const renderItem = ({ item, index }: { item: Player; index: number }) => {
    ...
    return (
      <View style={[styles.playerRow, { backgroundColor: bgColor, borderColor }]}>
        ...
        {item.countryFlag && (
          <Image source={{ uri: item.countryFlag }} style={styles.flagIcon} resizeMode="contain" />
        )}
